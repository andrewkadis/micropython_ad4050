...
Project Name: crypto_hmac

Description:  Application example demonstrating the Crypto device driver in HMAC cipher mode.


Overview:
=========
    This example demonstrates configuration of the Crypto device driver to perform a simple
    HMAC hash operation.

    The application performs the usual declarations and initializations, followed by a DMA
    driven HMAC crypto data transformation.
...
=====================
    Emulator connection required. This example requires a live debugger session to run because it uses
...
=====================
    The example performs the following operations and reports progress to the Terminal I/O window:
        1. Compute the HMAC hash of a message, 'message', using a 512-bit key, 'key'.
        2. Readback and verify the produced digest against th expected result, 'hmac'.
