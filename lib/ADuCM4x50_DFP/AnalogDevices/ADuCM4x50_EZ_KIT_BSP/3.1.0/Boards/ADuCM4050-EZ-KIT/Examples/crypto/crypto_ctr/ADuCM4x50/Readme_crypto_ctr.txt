...
Project Name: crypto_ctr

Description:  Application example demonstrating the Crypto device driver in ctr cipher mode.
...
=========
    ...
    The application performs the usual declarations and initializations, followed by a ctr crypto 
	data transformation.
    
	For CTR mode, known NIST input data is used for encryption and the encrypted data is 
...
=====================
	The example performs the following operations and reports progress to the Terminal I/O window:
		1. Encode and decode NIST data using CTR cipher mode. Test encrypted data against NIST standard.
		2. On failure, look for failure-specific messages.
		3. On success, look for message:
